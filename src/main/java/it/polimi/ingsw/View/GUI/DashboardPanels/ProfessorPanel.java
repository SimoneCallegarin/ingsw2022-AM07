...
public class ProfessorPanel extends JPanel {
    ...
    public ProfessorPanel(ModelStorage storage, int playerID, ArrayList<BufferedImage> professors) {
        ...
        GridLayout gridLayout = new GridLayout(professorGridRows,professorGridColumns);
        gridLayout.setHgap(professorHgap);
        setLayout(gridLayout);
        initializeProfessors(playerID,storage);
    }

    /**
     * Initializes the panel according to the professors state information in the ModelStorage.
     * It's also called on update, after the resetProfessors method to draw the professors on an empty panel.
     * @param playerID the player ID associated with this dashboard.
    ...
    private void initializeProfessors(int playerID, ModelStorage storage) {
        ...
        for(int i=0; i<5; i++){
            ...
            emptyPanel.setPreferredSize(professorDimension);
    ...
    /**
     * Resets this panel by removing every component on it.
     * It's called before the initializeProfessor method in order to place components on an empty panel.
     */
